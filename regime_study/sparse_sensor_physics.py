...
Sparse-sensor regime with phase-aware physics. Same 3-train/5-test
station split as sparse_sensor_baseline.py; training loss adds the
four phase-aware penalties from v5 (trend, onset_timing,
congestion_depth, recovery_timing). Measures whether physics-guided
regularization helps generalize to unseen detectors.
"""
...
OUT_DIR = "regime_study_output/sparse_sensor_physics_output"
...
TITLE_PREFIX = "Sparse-sensor physics"
...
def save_metrics(results, train_stations, test_stations):
    ...
    with open(os.path.join(OUT_DIR, "metrics.txt"), 'w') as f:
        f.write("Sparse-sensor physics (phase-aware losses, 5 seeds)\n")
        f.write("=" * 60 + "\n\n")
...
def save_phase_metrics(results):
    ...
    with open(os.path.join(OUT_DIR, "phase_metrics.txt"), 'w') as f:
        f.write("Sparse-sensor physics — Phase-Stratified Metrics\n")
        f.write("=" * 55 + "\n\n")

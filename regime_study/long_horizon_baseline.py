...
Long-horizon regime baseline: pure data-driven, no physics.
Extends baseline v6 architecture from OUTPUT_WINDOW=6 (30 min) to
OUTPUT_WINDOW=24 (120 min). 5 seeds. Paired with
long_horizon_physics.py to measure the delta from phase-aware
physics under accumulated-error stress.
"""
...
OUT_DIR = "regime_study_output/long_horizon_baseline_output"
...
# Plots (style consistent with baseline/physics output)
TITLE_PREFIX = "Long-horizon baseline"
...
def _horizon_xticks():
    ...
    # For 24-step horizon, show labels every 15 min (every 3rd bar)
...
def save_metrics(results):
    ...
    with open(os.path.join(OUT_DIR, "metrics.txt"), 'w') as f:
        f.write("Long-horizon baseline (pure data, 5 seeds)\n")
        f.write("=" * 50 + "\n\n")
...
def save_phase_metrics(results):
    ...
    with open(os.path.join(OUT_DIR, "phase_metrics.txt"), 'w') as f:
        f.write("Long-horizon baseline — Phase-Stratified Metrics\n")
        f.write("=" * 55 + "\n\n")

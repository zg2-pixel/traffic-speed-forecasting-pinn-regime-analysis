...
Baseline v2: same model as v1, but with early stopping and deepcopy
of best state. Per-station sliding windows retained.
"""
...
OUT_DIR = "baseline_output/v2_refactor_output"
...
def train_model(model, train_loader, val_loader):
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch + 1) % 5 == 0 or tag:
            print(f"Epoch {epoch+1:3d}/{EPOCHS}  train={train_loss:.5f}  val={val_loss:.5f}{tag}")
...
def plot_loss_curve(train_losses, val_losses):
    ...
    ax.set_title('v2 (simple GRU + early stopping) — Training Loss')
    ax.legend()
...
def save_metrics(rmse, mae, per_horizon_rmse, train_losses, val_losses, best_val):
    with open(os.path.join(OUT_DIR, "metrics.txt"), 'w') as f:
        f.write("Baseline v2 (simple GRU + early stopping)\n")
        f.write("=" * 45 + "\n\n")

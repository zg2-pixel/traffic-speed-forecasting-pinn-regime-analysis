...
Baseline v4 (main baseline): v3 with tuned hyperparameters and
seeded reproducibility. Hidden 64 -> 48, LR 1e-3 -> 3e-4,
early-stopping patience 15 -> 12, MIN_DELTA added, fixed seed.
"""
...
OUT_DIR = "baseline_output/v4_final_output"
...
def plot_loss_curve(train_losses, val_losses, lr_history):
    ...
    ax1.set_title('v4 — Training Loss')
    ax1.legend()
...
def plot_pred_vs_truth(truths, preds):
    ...
    axes[0].set_title('v4 — Prediction vs Truth (t+5 min)')
    axes[0].legend()
    ...
    axes[1].set_title('v4 — Prediction vs Truth (t+30 min)')
    axes[1].legend()
...
def plot_horizon_rmse(per_horizon_rmse):
    ...
    ax.set_title('v4 — RMSE by Horizon')
...
def plot_attention(attns):
    ...
    ax.set_title('v4 — Temporal Attention')
...
def save_metrics(rmse, mae, per_horizon_rmse, train_losses, val_losses, best_val):
    with open(os.path.join(OUT_DIR, "metrics.txt"), 'w') as f:
        f.write("Baseline v4 (final: tuned hyperparameters + seeded)\n")
        f.write("=" * 55 + "\n\n")

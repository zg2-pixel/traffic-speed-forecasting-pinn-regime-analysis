...
Reads PeMS 5-minute station data from data/raw/, filters to selected
I-880 Southbound stations, downloads hourly weather from Open-Meteo,
adds time features, and merges everything into a clean CSV that
feeds event_labeling.py.
...
# Candidate stations on I-880 Southbound (San Jose -> Fremont).
# 10 IDs are listed here; after the quality filter in clean_data()
# (avg_speed > 0 and pct_observed >= 50), 2 get dropped and 8 are
# retained in the final traffic_clean.csv.
SELECTED_STATIONS = {
...
def download_weather():
    ...
    # Handle SSL certificate issues on Mac. Some Python installs on macOS
    # ship without the system root CA bundle, which makes urlopen fail on
    # HTTPS. Fall back to an unverified context only if the normal one
    # raises. Safe here because Open-Meteo's archive API is public and
    # returns JSON only.
    try:
...
if __name__ == "__main__":
    print("=" * 60)
    print("Preprocessing: PeMS + Weather -> traffic_clean.csv")
    print("=" * 60)

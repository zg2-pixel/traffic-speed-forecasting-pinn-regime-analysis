...
Physics v5 (event-focused): physics losses targeted at specific event
phases (onset / congested / recovery). Four phase-aware penalties:
  1. trend           - onset should drop, recovery should rise
  2. onset_timing    - penalize late onset detection
  3. congestion_depth- penalize overprediction during congested phase
                       (extra weight when true speed <= 45 mph)
  4. recovery_timing - penalize late recovery detection

Built on baseline v6 (INPUT=3, speed+occupancy only, phase labels).
5 seeds. Generates single-seed plots, 5-seed aggregated plots, and
phase-stratified products in line with baseline v6 output layout.
"""
...
OUT_DIR = "physics_output/v5_event_focused_output"
...
# Phase-aware physics loss weights (from physics.py)
...
# Phase-aware physics losses (ported verbatim from physics.py)
def compute_phase_losses(pred_scaled, y_scaled, phase_ids, x_scaled,
    ...
    # (1) Trend: in onset speed should drop (not rise), in recovery rise (not drop)
    ...
    # (2) Onset timing: penalize overprediction at step 0 and shallow early drop
    ...
    # (3) Congestion depth: penalize overprediction during congested phase;
    # extra weight when true speed is below low-speed threshold
    ...
    # (4) Recovery timing: penalize underprediction at step 0 and shallow early rise
...
def prepare_data():
    ...
    for sid in sorted(df['station'].unique()):
        ...
        for split_name, lst in [('train', train_list), ('val', val_list), ('test', test_list)]:
            ...
            f, s, p = feat[sel], sp_scaled[sel], ph[sel]
...
def train_one_seed(seed, loaders, speed_mean, speed_std, save_best=False):
    ...
    # Phase-stratified metrics (flatten across horizons)
...
def plot_loss_curve(r):
    ...
    ax1.set_title(f'v5 event-focused (seed {r["seed"]}) — Training Loss')
    ax1.legend()
...
def plot_physics_loss_breakdown(r):
    ...
    ax.set_title(f'v5 (seed {r["seed"]}) — Phase-Aware Loss Components')
...
def plot_loss_curve_5seed(results):
    ...
    ax1.set_xlabel('Epoch')
    ax1.set_ylabel('Total Loss')
    ax1.set_title('v5 event-focused — 5-seed Training Loss')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
...
def plot_phase_rmse_bars(results):
    ...
    ax.set_ylabel('RMSE (mph)')
    ax.set_title('v5 — RMSE by Event Phase (5-seed mean ± std)')
    ax.grid(True, alpha=0.3, axis='y')
    plt.tight_layout()
...
def plot_phase_horizon_rmse(results):
    ...
    ax.set_xlabel('Prediction Horizon (min)')
    ax.set_ylabel('RMSE (mph)')
    ax.set_title('v5 — RMSE by Phase × Horizon (5-seed mean ± std)')
    ax.set_xticks(horizons)
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
...
def plot_pred_vs_truth_phase_colored(r):
    ...
    n = min(500, len(preds))
    fig, axes = plt.subplots(2, 1, figsize=(14, 8), sharex=True)
    for (ax, h_idx, title) in [(axes[0], 0, 't+5 min'), (axes[1], -1, 't+30 min')]:
        ph = phases[:n, h_idx]
        run_start = 0
        cur_phase = ph[0]
        for i in range(1, n):
        ...
        ax.set_title(f'v5 (seed {r["seed"]}) — {title} (phase-colored)')
...
def save_metrics(results):
    ...
    with open(os.path.join(OUT_DIR, "metrics.txt"), 'w') as f:
        f.write("Physics v5 (event-focused, phase-aware losses, 5 seeds)\n")
        f.write("=" * 55 + "\n\n")
...
def save_phase_metrics(results):
    ...
    with open(os.path.join(OUT_DIR, "phase_metrics.txt"), 'w') as f:
        f.write("v5 Phase-Stratified Metrics (5-seed mean +/- std)\n")
        f.write("=" * 55 + "\n\n")

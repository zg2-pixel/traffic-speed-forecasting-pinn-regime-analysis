...
Physics v1: three soft penalties applied to all predictions.
  - Acceleration limit:   |v(t) - v(t-1)| <= 15 mph
  - Speed range:           0 <= v <= 85 mph
  - Flow-based congestion: flow > 450 (veh/5min) => v <= 50 mph

Built on baseline v4 (GRU + temporal attention, 12 features, INPUT=12).
Penalties use small fixed weights (physics as gentle regularization,
not as competing objectives).
"""
...
OUT_DIR = "physics_output/v1_flow_penalty_output"
...
# Dataset: returns X, y, and future flow (for flow-based penalty)
class StationDataset(Dataset):
    def __init__(self, features, targets, flow_orig, input_window, output_window):
        self.X, self.y, self.flow = [], [], []
        n = len(features)
        for i in range(n - input_window - output_window + 1):
            ...
            self.flow.append(flow_orig[i + input_window:i + input_window + output_window])
        self.X = torch.FloatTensor(np.array(self.X))
        self.y = torch.FloatTensor(np.array(self.y))
        self.flow = torch.FloatTensor(np.array(self.flow))

    def __len__(self):
        return len(self.X)

    def __getitem__(self, idx):
        return self.X[idx], self.y[idx], self.flow[idx]
...
# Physics losses
def compute_physics_losses(pred_scaled, flow_orig, speed_mean, speed_std):
    pred_mph = pred_scaled * speed_std + speed_mean

    # Acceleration: penalize |dv| beyond ACC_LIMIT
    ...
    # Speed range: penalize excursions below 0 or above 85
    below = torch.clamp(SPEED_MIN - pred_mph, min=0)
    above = torch.clamp(pred_mph - SPEED_MAX, min=0)
    L_speed = (below + above).mean()

    # Flow-based congestion: flow > FLOW_CONG => v <= V_CONG
    is_cong = (flow_orig > FLOW_CONG).float()
...
def prepare_data():
    ...
    flow_orig_all = df['total_flow'].values.copy()
    ...
    for sid in stations:
        ...
        sdf_flow = flow_orig_all[mask.values]
        ...
        for sel, lst in [(s_train, train_list), (s_val, val_list), (s_test, test_list)]:
            f, t, fl = feat[sel], tgt[sel], sdf_flow[sel]
            if len(f) > INPUT_WINDOW + OUTPUT_WINDOW:
                lst.append(StationDataset(f, t, fl, INPUT_WINDOW, OUTPUT_WINDOW))
...
def evaluate(model, test_loader, speed_mean, speed_std):
    ...
    # Constraint-violation stats on test set
    dv = np.abs(preds[:, 1:] - preds[:, :-1])
    acc_viol_rate = float((dv > ACC_LIMIT).mean())
    speed_viol_rate = float(((preds < SPEED_MIN) | (preds > SPEED_MAX)).mean())
    is_cong = (flows > FLOW_CONG)
    cong_viol_rate = float(((preds > V_CONG) & is_cong).sum() / max(is_cong.sum(), 1))
    viol = {
        'acc_violation_rate': acc_viol_rate,
        'speed_violation_rate': speed_viol_rate,
        'cong_violation_rate': cong_viol_rate,
...
def plot_loss_curve(history):
    ...
    ax1.set_title('v1 (flow penalty) — Training Loss')
    ax1.legend()
...
def plot_physics_losses(history):
    ...
    ax.plot(ep, history['train_cong'], label='Flow-based congestion', linewidth=1.2)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Unweighted component loss')
    ax.set_title('v1 — Physics Loss Components (training)')
...
def plot_attention(attns):
    ...
    ax.set_title('v1 — Temporal Attention')
...
def save_metrics(rmse, mae, per_horizon_rmse, history, best_val, viol):
    with open(os.path.join(OUT_DIR, "metrics.txt"), 'w') as f:
        f.write("Physics v1 (flow-based 3-penalty)\n")
        f.write("=" * 45 + "\n\n")
        ...
        f.write(f"  features = {len(FEATURE_COLS)}  (includes total_flow)\n")
        ...
        f.write(f"  flow_congestion: flow > {FLOW_CONG} => v <= {V_CONG} mph  (lambda = {LAMBDA_CONG})\n\n")
...
def save_violation_stats(viol):
    with open(os.path.join(OUT_DIR, "violation_stats.txt"), 'w') as f:
        f.write("v1 — Test-set Constraint Violation Stats\n")
        f.write("=" * 45 + "\n\n")
        ...
        f.write(f"Flow-congestion violations (cong & v>{V_CONG}): "

"""
Step 3: Physics-Informed GRU Model
====================================
...
Outputs in outputs_physics_v1/:
  - loss_curve_physics_v1.png
  - prediction_vs_truth_physics_v1*.png
  - metrics_physics_v1.txt
  - constraint_violations_v1.png
"""
...
OUT_DIR = "outputs_physics_v1"
os.makedirs(OUT_DIR, exist_ok=True)

# Same model config as baseline
INPUT_WINDOW = 12
OUTPUT_WINDOW = 6
HIDDEN_SIZE = 64
NUM_LAYERS = 2
...
# Physics constraint parameters (in ORIGINAL scale, mph)
# These will be converted to standardized scale during training

A_MAX_MPH = 15.0      # max speed change per 5-min interval (mph)
V_MAX_MPH = 85.0      # max reasonable freeway speed (mph)
V_MIN_MPH = 0.0       # min speed
FLOW_CONGESTION = 180  # flow threshold for congestion (vehicles/5min)
V_CONGESTION_MPH = 50.0  # max speed when congested (mph)

# Loss weights
LAMBDA_ACC = 0.1       # acceleration constraint weight
LAMBDA_SPEED = 0.1     # speed range constraint weight
LAMBDA_CONG = 0.05     # congestion constraint weight


# Dataset (same as baseline, but also returns flow)

class TrafficDatasetPhysics(Dataset):
    """Sliding window dataset that also returns flow for congestion constraint."""
    def __init__(self, features, targets, flow, input_window, output_window):
        self.features = features
        self.targets = targets
        self.flow = flow  # flow values (original scale) for congestion check
...
def compute_physics_losses(pred, flow, speed_mean, speed_std, flow_mean, flow_std):
    """
    Compute three physics constraint penalty terms.

    pred: (batch, output_window) — predicted speed in STANDARDIZED scale
    flow: (batch, output_window) — flow in ORIGINAL scale
    speed_mean, speed_std: for converting between scales
    flow_mean, flow_std: for reference (flow is already original scale)

    Returns: L_acc, L_speed, L_cong (all scalar tensors)
    """
    # Convert predictions to original scale (mph) for physics checks
    pred_mph = pred * speed_std + speed_mean

    # Constraint 1: Acceleration limit
    # |v(t) - v(t-1)| <= A_MAX_MPH
    # Penalize the excess beyond A_MAX_MPH
    if pred_mph.shape[1] > 1:
        speed_diff = torch.abs(pred_mph[:, 1:] - pred_mph[:, :-1])  # (batch, output_window-1)
        acc_violation = torch.clamp(speed_diff - A_MAX_MPH, min=0)  # only penalize violations
    ...
    # 0 <= v(t) <= V_MAX_MPH
    below_min = torch.clamp(V_MIN_MPH - pred_mph, min=0)   # penalty for v < 0
    above_max = torch.clamp(pred_mph - V_MAX_MPH, min=0)    # penalty for v > 85
    L_speed = (below_min + above_max).mean()

    # Constraint 3: Congestion-speed consistency
    # If flow > FLOW_CONGESTION, then v should be <= V_CONGESTION_MPH
    is_congested = (flow > FLOW_CONGESTION).float()  # (batch, output_window)
...
def prepare_data():
    ...
    # Save original flow values BEFORE scaling (for congestion constraint)
    ...
    flow_std = scaler.scale_[flow_idx]

    df[FEATURE_COLS] = scaler.transform(df[FEATURE_COLS])

    # Build datasets per station
    train_datasets, val_datasets, test_datasets = [], [], []
    test_meta = []

    for sid in stations:
        sdf = df[df['station'] == sid].copy().reset_index(drop=True)
        sdf_idx = df[df['station'] == sid].index  # original indices for flow lookup

        features = sdf[FEATURE_COLS].values
        targets = sdf[TARGET_COL].values
        flow_orig = original_flow[sdf_idx]  # original scale flow
    ...
    return train_loader, val_loader, test_loader, test_meta, speed_mean, speed_std, flow_mean, flow_std


# Training with physics constraints

def train_model(model, train_loader, val_loader, speed_mean, speed_std, flow_mean, flow_std):
    ...
    for epoch in range(EPOCHS):
        ...
        for x, y, flow in train_loader:
            ...
            # Physics losses
            L_acc, L_speed, L_cong = compute_physics_losses(
                pred, flow, speed_mean, speed_std, flow_mean, flow_std
            )

            # Total loss
            loss = L_data + LAMBDA_ACC * L_acc + LAMBDA_SPEED * L_speed + LAMBDA_CONG * L_cong
        ...
        # Validate (only data loss for fair comparison)
        model.eval()
        val_loss = 0
        with torch.no_grad():
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(OUT_DIR, "best_model_physics_v1.pt"))

        if (epoch + 1) % 5 == 0 or epoch == 0:
            print(f"  Epoch {epoch+1}/{EPOCHS} — Data: {train_loss:.6f}, "
                  f"Acc: {epoch_acc_loss/n_samples:.4f}, Speed: {epoch_speed_loss/n_samples:.4f}, "
                  f"Cong: {epoch_cong_loss/n_samples:.4f}, Val: {val_loss:.6f}")

    model.load_state_dict(torch.load(os.path.join(OUT_DIR, "best_model_physics_v1.pt"), weights_only=True))
...
def plot_loss_curve(train_losses, val_losses):
    ...
    ax.set_title('Physics-Informed GRU (v1) — Training and Validation Loss')
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, "loss_curve_physics_v1.png"), dpi=150, bbox_inches='tight')
    plt.close()
    print("Saved: loss_curve_physics_v1.png")
...
def plot_physics_losses(train_physics):
    ...
    plt.suptitle('Physics Constraint Losses During Training (v1)', y=1.02)
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, "constraint_losses_v1.png"), dpi=150, bbox_inches='tight')
    plt.close()
    print("Saved: constraint_losses_v1.png")
...
def plot_predictions(test_meta, model, speed_mean, speed_std):
    ...
    for meta in test_meta[:3]:
        ...
        ax.plot(time_list[:n_points], preds_list[:n_points],
                label='Physics-Informed', color='#ff7f0e', linewidth=1, alpha=0.8)
        ax.set_xlabel('Time')
        ax.set_ylabel('Speed (mph)')
        ax.set_title(f'Station {sid} — Physics v1 Prediction vs Ground Truth')
        ax.legend()
        ax.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(OUT_DIR, f"prediction_vs_truth_physics_{sid}_v1.png"),
                    dpi=150, bbox_inches='tight')
        plt.close()
        print(f"Saved: prediction_vs_truth_physics_{sid}_v1.png")


def save_metrics(rmse, mae, train_losses, val_losses):
    with open(os.path.join(OUT_DIR, "metrics_physics_v1.txt"), 'w') as f:
        f.write("=" * 50 + "\n")
        f.write("Physics-Informed GRU Model — Test Metrics\n")
        f.write("=" * 50 + "\n\n")
    ...
    print("Saved: metrics_physics_v1.txt")
...
if __name__ == "__main__":
    print("=" * 60)
    print("Step 3: Train Physics-Informed GRU")
    print("=" * 60)
    ...
    print(f"  Congestion threshold: flow > {FLOW_CONGESTION} veh/5min => speed <= {V_CONGESTION_MPH} mph")
    print(f"  Weights: lambda_acc={LAMBDA_ACC}, lambda_speed={LAMBDA_SPEED}, lambda_cong={LAMBDA_CONG}")

    # 1. Prepare data
    print("\n[1/4] Preparing data...")
    train_loader, val_loader, test_loader, test_meta, speed_mean, speed_std, flow_mean, flow_std = prepare_data()

    # 2. Train model
    print(f"\n[2/4] Training Physics-Informed GRU ({EPOCHS} epochs)...")
    model = GRUModel(
    ...
    train_losses, val_losses, train_physics = train_model(
        model, train_loader, val_loader, speed_mean, speed_std, flow_mean, flow_std
    )
    ...
    print("\n" + "=" * 60)
    print("Done! Check outputs_physics_v1/ for results.")
